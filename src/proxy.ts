...
const PROTECTED_ROUTES: Record<string, string[]> = {
  ...Object.fromEntries(Object.values(BO_ROUTES).map((route) => [route, [UserRole.SUPER_ADMIN]])),
};

const PROTECTED_PREFIXES = Object.keys(PROTECTED_ROUTES);
const TOTP_ROUTE = APP_ROUTES._2FA;
...
export async function proxy(request: NextRequest) {
  ...
  if (pathname === TOTP_ROUTE && !totpCookie) {
    return redirectTo(request, APP_ROUTES.ROOT);
  }
