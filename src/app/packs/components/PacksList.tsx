...
import { PacksModule } from "@/store/state-management";
...
export const PackList = () => {
  ...
  const [selectedPack, setSelectedPack] =
    useState<MODELS.IPacksResponse | null>(null);
  const {
    data: packsData,
    isLoading,
    refetch,
  } = PacksModule.getAllPacksQueries({});
  ...
  const packsColumns: ColumnsDataTable[] = [
    ...
    },
    {
      ...
      accessor: "fullObject",
      cell: (row) => (
        <BaseFormatNumber value={row?.commissionRate} style="percent" />
      ),
    },
    {
      header: "Status",
    ...
    },
    {
      ...
      cell: (subscriptions: string[]) => (
        <BaseText>{subscriptions?.length}</BaseText>
      ),
    },
    {
