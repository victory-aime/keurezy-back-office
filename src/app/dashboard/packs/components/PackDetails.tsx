import { BaseAccordion, BaseDrawer, BaseText, Icons, ModalOpenProps } from '_components/custom';
import { Span, VStack } from '@chakra-ui/react';
import { groupedFeatures } from './utils/groupedFeatured';
import { useTranslation } from 'react-i18next';

export const PackDetails = ({ isOpen, onChange, data, isLoading }: ModalOpenProps) => {
  const { t } = useTranslation();
  const accordionItems = Object.entries(groupedFeatures(data!)).map(([category, features]) => ({
      ...
      <VStack align="start">
        {features.map((pf) => (
          <BaseText key={pf.id}>
            ...
            {pf.enabled ? (pf.limit ? `Limite à: ${pf.limit}` : 'Illimité') : 'Désactivé'}
          </BaseText>
        ))}
      </VStack>
    ),
  ...
  return (
    ...
    >
      <BaseText>
        Liste des fonctionnalités: <Span color={'primary.500'}>{data?.name} </Span>
      </BaseText>
      <VStack mt={4}>
        <BaseAccordion items={accordionItems} multipleOpen isLoading={isLoading} />
      </VStack>
    </BaseDrawer>
  );
};
